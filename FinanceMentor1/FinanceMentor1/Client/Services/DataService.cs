    ...
    public class DataService : IDataService
    {
        ...
        public async Task<ICollection<YearlyItem>> LoadCurrentYearEarnings()
        {
            ...
            return data.Where(earning => earning.Date >= new DateTime(_currentYear, 1, 1)
                ...
                .OrderBy(earning => earning.Key).Select(earning => new YearlyItem
                {
        ...
        {
            var data = await _httpClient.GetFromJsonAsync<Earning[]>("api/Earnings");
            return data.Where(earning => earning.Date.Month == month).GroupBy(earning => earning.Category)
        ...
        public async Task<ThreeMonthsData> LoadLast3MonthsExpenses()
        {
            return new ThreeMonthsData
            {
                CurrentMonth = new MonthlyData
                {
                    Data = await GetMonthlyExpenses(DateTime.Today.Month, DateTime.Today.Year),
                    Label = GetMonthAsText(DateTime.Today.Month, DateTime.Today.Year)
                },
                LastMonth = new MonthlyData
                {
                    Data = await GetMonthlyExpenses(DateTime.Today.AddMonths(-1).Month, DateTime.Today.AddMonths(-1).Year),
                    Label = GetMonthAsText(DateTime.Today.AddMonths(-1).Month, DateTime.Today.AddMonths(-1).Year)
                },
                PreviousMonth = new MonthlyData
                {
                    Data = await GetMonthlyExpenses(DateTime.Today.AddMonths(-2).Month, DateTime.Today.AddMonths(-2).Year),
                    Label = GetMonthAsText(DateTime.Today.AddMonths(-2).Month, DateTime.Today.AddMonths(-2).Year)
                },
            };
        }
        ...
        {
            var data = await _httpClient.GetFromJsonAsync<Earning[]>("api/Expenses");
            return data.Where(expense => expense.Date.Month == month).GroupBy(expense => expense.Category)
